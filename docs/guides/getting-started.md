...
This guide walks through a generic `fake-app` lane. It does not require private infrastructure.
...
```bash
mkdir -p /repo/fake-app/.agent-wave/lanes
mkdir -p /repo/fake-app/.agent-wave/state
...
Only merge after the lane, trace, safety scan, and validation evidence are coherent. Agent Wave is designed for supervised automation, not autonomous branch integration.
